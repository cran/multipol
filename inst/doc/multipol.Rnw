...
by~\citet{venables2001} as an example of S3 classes.
...
of~\citeyear{venables2008}, but C++ includes a ``map''
...
possible with the current implementation of~S3 or
S4~\citep{venables2008}.  Recent experimental work by~\citet{west2008}
exhibits a proof-of-concept in C++ which might form the back end of an
...
array, for which the ``map'' class of the C++ language is ideally
